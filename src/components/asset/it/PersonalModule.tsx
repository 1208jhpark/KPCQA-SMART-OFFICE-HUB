...
export default function PersonalModule() {
  ...
  const stats = useMemo(() => {
    ...
    return { verified, typeCounts, total: myAssets.length, replaceable: myAssets.filter(a => getAssetLogic(a).isTargetCount).length };
  }, [myAssets, auditBaseline]);
  ...
  const filteredAssets = useMemo(() => {
    return myAssets.filter(a => {
      ...
      return matchSearch && matchCategory && matchType && matchReplace && matchUnverified;
    });
  }, [myAssets, searchQuery, colFilters, showReplaceableOnly, showUnverifiedOnly, auditBaseline]);
  
  const paginatedAssets = filteredAssets.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
  const totalPages = Math.max(1, Math.ceil(filteredAssets.length / itemsPerPage));
  
  const availableYears = useMemo(() => {
    const years = requests.filter(r => r.requester === currentUser?.name).map(r => r.requestDate.substring(0, 4));
  ...
  const myReqs = useMemo(() => {
    return requests
      // 🚀 [해결] selectedYear가 'ALL'일 경우 조건없이 전부 반환
      .filter(r => r.requester === currentUser?.name && (selectedYear === 'ALL' || r.requestDate.startsWith(selectedYear)))
      .sort((a, b) => b.id.localeCompare(a.id));
  }, [requests, currentUser, selectedYear]);
  
  const paginatedRequests = myReqs.slice((historyPage - 1) * itemsPerPage, historyPage * itemsPerPage);
  const requestTotalPages = Math.max(1, Math.ceil(myReqs.length / itemsPerPage));
  ...
  const handleExcelDownload = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "My_Assets"); 
    XLSX.writeFile(wb, `나의업무자산현황_${currentUser?.name}_${new Date().toISOString().split('T')[0]}.xlsx`);
  };

  // 🚀 [추가] 나의 요구사항 전송 이력용 엑셀 다운로드 함수
  const handleRequestExportExcel = () => {
    if (myReqs.length === 0) return alert('다운로드할 요구사항 이력 데이터가 없습니다.');
    
    const exportData = myReqs.map((req, idx) => ({
      'NO': myReqs.length - idx,
    ...
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "나의요구사항이력");
    XLSX.writeFile(wb, `나의요구사항_${currentUser?.name}_${selectedYear === 'ALL' ? '전체' : selectedYear}년.xlsx`);
  };
    ...
    <div className="p-4 space-y-4 bg-[#F8FAFC] min-h-screen font-sans text-slate-900 text-[11px] relative">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white border border-slate-200 p-5 shadow-sm rounded-xl flex flex-col justify-between">
          <div>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">실사 인증 현황</p>
        ...
        <div className="bg-slate-900 p-5 shadow-md rounded-xl flex flex-col justify-between col-span-2 relative overflow-hidden">
          <div className="flex justify-between items-start z-10">
             <div>
               <h2 className="text-white text-xl font-black tracking-tight mb-1">나의 IT·업무자산 운영 현황</h2>
      ...
      <div className="bg-white border border-slate-200 px-5 h-14 shadow-sm rounded-xl flex gap-3 items-center">
        <input 
          type="text" 
          placeholder="[통합검색] 자산번호, 모델명, S/N를 입력하세요" 
          value={searchQuery} 
        ...
        <button onClick={() => selectedIds.size > 0 && setShowRequestModal(true)} className={`px-6 py-2 font-black rounded-lg transition-all shadow-sm ${selectedIds.size > 0 ? 'bg-indigo-600 text-white hover:bg-indigo-700' : 'bg-slate-100 text-slate-400 border border-slate-200'}`}>요구사항 전송 ({selectedIds.size})</button>
      </div>
          ...
          <table className="w-full text-left border-collapse min-w-[2100px] table-fixed">
            <thead>
              <tr className="h-14 bg-slate-900 text-slate-400 uppercase font-bold border-b border-slate-800">
                ...
                <th className="p-3 w-[80px] text-center sticky left-[300px] bg-slate-900 z-30">범주</th>
                <th className="p-3 w-[120px] text-center sticky left-[380px] bg-slate-900 z-30 text-blue-400 border-r-2 border-slate-700">{typeLabel}</th>
              ...
              {paginatedAssets.map((a, idx) => {
                ...
                return (
                  <tr key={a.id} className={`hover:bg-slate-50 h-12 transition-all font-bold ${isSelected ? 'bg-indigo-50/30' : ''}`}>
                    ...
                    <td className={`p-2 text-center text-slate-500 sticky left-[300px] z-20 ${stickyBg}`}>{a.category}</td>
                    <td className={`p-2 text-center text-blue-600 sticky left-[380px] z-20 font-black border-r-2 border-slate-200 ${stickyBg}`}>{a.it_type}</td>
      ...
      <div className="bg-white border border-slate-200 shadow-sm rounded-xl overflow-hidden mt-6">
        <div onClick={() => setIsHistoryOpen(!isHistoryOpen)} className="px-5 h-14 flex justify-between items-center cursor-pointer bg-slate-800 text-white hover:bg-slate-700 transition-all">
          <h3 className="text-xs font-black uppercase tracking-tighter flex items-center gap-2">📤 나의 요구사항 전송 이력 ({myReqs.length}건)</h3>
              ...
              <table className="w-full text-left text-[11px] border-collapse min-w-[1200px] table-fixed">
                ...
                <tbody className="divide-y divide-slate-100">
                  {paginatedRequests.length === 0 ? (
                    <tr>
                      <td colSpan={7} className="p-8 text-center text-slate-400 font-bold">해당 연도의 요구사항 이력이 없습니다.</td>
                    </tr>
                  ) : (
          ...
          <div className="bg-white w-[500px] border border-slate-300 shadow-2xl p-8 rounded-[2.5rem]">
            ...
            <div className="mb-4">
              ...
              <div className="max-h-24 overflow-y-auto bg-slate-50 border rounded-xl p-2 text-[10px] space-y-1 shadow-inner">
                {myAssets.filter(a => selectedIds.has(a.id)).map(a => (
            ...
            <div className="flex gap-2 mt-6">
              ...
              <button onClick={() => {
                ...
                const targetAssets = myAssets.filter(a => selectedIds.has(a.id));
